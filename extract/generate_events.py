...
def generate_events(
    ...
    def add_event(
        event_ts, user_id, session_id, event_name, props, exp_key=experiment_key, variant=None
    ):
    ...
    for u in user_ids:
        ...
        first_seen = start + timedelta(
            seconds=random.randint(0, int((end - start).total_seconds()))
        )

        for s in range(n_sessions):
            # Session start time jittered forward from first_seen
            session_ts = first_seen + timedelta(
                days=random.random() * (n_days - 1), minutes=random.random() * 1440
            )
            ...
            for _ in range(n_views):
                ...
                add_event(
                    ...
                    session_id,
                    "product_view",
                    {"product_id": pid, "category": random.choice(categories), "price": price},
                    variant=v,
                )
            ...
            if random.random() < min(base_checkout_p, 0.40):
                ...
                add_event(
                    ...
                    session_id,
                    "checkout_started",
                    {"cart_value": cart_value, "items": items},
                    variant=v,
                )
